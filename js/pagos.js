    ...
    if (item.PAGOS && item.PAGOS.length > 0) {
      item.PAGOS.forEach((pago) => {
        ...
        columnsToShow.forEach((column) => {
          ...
          cell.textContent = item[column] || pago[column];
...
const mostrarPagos = (datos) => {
  const columnsToShow = ["REF_PAGO", "TORRE", "APTO", "FECHA", "MONTO"];
...
const filtrarPagos = async (e) => {
  ...
  try {
    ...
    const pagosFiltrados = datos.filter((pago) => {
      ...
      if (campo === "FECHA" || campo === "MONTO") {
        return pago.PAGOS.some(
          (detallePago) => String(detallePago[campo]).trim() === filtro
        );
      } else {
...
const cPagos = document.getElementById("consultaPagos");
cPagos.addEventListener("submit", filtrarPagos);
